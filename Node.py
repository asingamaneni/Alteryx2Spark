...
class NodeElement(object):
    ...
    def __init__(self, node, root):
        ...
        if self.plugin == "AlteryxBasePluginsGui.Join.Join":
            join_data = (
                node.find("Properties").find("Configuration").findall("JoinInfo")
            )
            ...
            self.description = (
                node.find("Properties").find("Configuration").find("Caption").text
            )

        elif self.plugin == "AlteryxSpatialPluginsGui.Summarize.Summarize":
            self.summarize_fields = (
                node.find("Properties").find("Configuration").find("SummarizeFields")
            )
            ...
            for field in self.summarize_fields:
                ...
                elif field["action"] == "Concat" or field["action"] == "Sum":
                    aggregations.append(
                        f'sum("{field["field"]}").alias("{field["rename"]}")'
                    )
            ...
            if group_by and aggregations:
                ...
                self.sparkquery = (
                    f"df{self.tool_id} = df.groupBy({', '.join(group_by)})"
                )
            elif aggregations:
                self.sparkquery = (
                    f"df{self.tool_id} = df.agg({', '.join(aggregations)})"
                )
            ...
            for e in self.select_fields:
                if e["selected"] == "True":
                    ...
                    if "rename" in e:
                        ...
                        # Replace *Unknown with * to select all other fields
                        if e["field"] == "*Unknown":
                            e["field"] = "*"
            ...
            self.sparkquery = (
                "df" + self.tool_id + " = df.selectExpr(" + self.sparkquery[:-1] + ")"
            )
        ...
        #     self.select_fields = None
            ...
            for e in self.sort_fields:
                if e["order"] == "Ascending":
                    ...
                    self.sparkquery = (
                        self.sparkquery + 'desc("' + e["field"] + '")' + ","
                    )

            self.sparkquery = (
                "df" + self.tool_id + " = df.orderBy(" + self.sparkquery[:-1] + ")"
            )
            ...
            self.filter_fields = (
                node.find("Properties")
                .find("Annotation")
                .find("DefaultAnnotationText")
                .text
            )

            cond = (
                self.filter_fields.replace("[", "")
                .replace("]", "")
                .replace(" = ", " == ")
                .replace('"', "'")
            )
        ...
        #     self.select_fields = None
            ...
            self.filter_fields = (
                node.find("Properties").find("Configuration").find("File")
            )
            ...
            and node.find("Properties").find("Configuration").find("Caption").text
            == "Delta tool - DO NOT MODIFY"
        ):
            self.delta = node.find("Properties").find("Configuration").findall("Value")
            c = 0
            self.deltaquery = ""
                ...
                if c == 0:
                    for m in field.text.split(","):
                        if m.endswith("True") and c == 0:
                            self.deltaquery = (
                                self.deltaquery + '"' + m.split("=")[0] + '",'
                            )
                    self.sparkquery = (
                        "df"
                        + self.tool_id
                        + '_new=ldfs.as("a").join(rdfs.as("b"), Seq('
                        + self.deltaquery[:-1]
                        + '),"left anti")\n'
                    )
                    self.sparkquery = (
                        ...
                        + "df"
                        + self.tool_id
                        + '_delete=rdfs.as("a").join(ldfs.as("b"), Seq('
                        + self.deltaquery[:-1]
                        + '),"left anti")\n'
                    )
                    self.sparkquery = (
                        ...
                        + "\n df"
                        + self.tool_id
                        + '_change=ldfs.as("a").join(rdfs.as("b"), Seq('
                        + self.deltaquery[:-1]
                        + '),"inner")'
                    )
                    c = 1
                    ...
                    self.sparkquery = (
                        self.sparkquery + ".where(" + self.deltaquery[:-3] + ")"
                    )
        ...
        for connection in root.find("Connections").iter("Connection"):
            ...
            if dest == self.tool_id:
                ...
                if (
                    origin
                    and connection.find("Destination").attrib.get("Connection")
                    == "Left"
                ):
                    self.sparkquery = (
                        self.sparkquery.replace("ldfs", "df" + origin)
                        if self.sparkquery
                        else None
                    )
                if (
                    ...
                    self.sparkquery = (
                        self.sparkquery.replace("rdfs", "df" + origin)
                        if self.sparkquery
                        else None
                    )
                if (
                    ...
                    self.sparkquery = (
                        self.sparkquery.replace("ldfs", "df" + origin + "_new")
                        if self.sparkquery
                        else None
                    )
                if (
                    ...
                    self.sparkquery = (
                        self.sparkquery.replace("rdfs", "df" + origin + "_change")
                        if self.sparkquery
                        else None
                    )
                if (
                ...
                if (
                    origin
                    and connection.find("Origin").attrib.get("Connection")
                    == "New Records"
                ):
                    self.sparkquery = (
                        self.sparkquery.replace("df.", "df" + origin + "_new.")
                        if self.sparkquery
                        else None
                    )
                if (
                    ...
                    self.sparkquery = (
                        self.sparkquery.replace("df.", "df" + origin + "_change.")
                        if self.sparkquery
                        else None
                    )
                ...
                if origin:
                    if self.sparkquery == "":
                        ...
                        self.sparkquery
                    self.sparkquery = (
                        self.sparkquery.replace("df.", "df" + origin + ".")
                        if self.sparkquery
                        else None
                    )
