...
def process_yxmd(file_content):
    ...
    # Initialize a counter for DataFrame names
    df_counter = 1
    df_names = {}

    # Assign DataFrame names only to nodes with non-empty "Spark Query"
    for tool_id in sorted_nodes:
        node_data = next(item for item in lst if item["Tool ID"] == tool_id)
        if node_data.get("Spark Query"):
            print("\n")
            print(node_data["Plugin"])
            print(node_data["Spark Query"])
...
def main():
    ...
    # read from yxmd file into a string
    with open("ASN_Edit_AmazonS3_V3.yxmd", "r") as file:
    ...
    pd.DataFrame(processed_data).to_csv("output.csv", index=False)
